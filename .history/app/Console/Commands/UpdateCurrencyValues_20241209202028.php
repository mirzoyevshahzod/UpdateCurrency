...
class UpdateCurrencyValues extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $allCurrencies = [
                ['name' => 'USD', 'value' => $valuesFromSite1['USD'] ?? null, 'date'],
                ['name' => 'RUB', 'value' => $valuesFromSite1['RUB'] ?? null, 'date'],
                ['name' => 'EUR', 'value' => $valuesFromSite1['EUR'] ?? null, 'date'],
                ['name' => 'CBU-USD', 'value' => $valuesFromSite2['USD'] ?? null, 'date'],
                ['name' => 'CBU-RUB', 'value' => $valuesFromSite2['RUB'] ?? null, 'date'],
                ['name' => 'CBU-EUR', 'value' => $valuesFromSite2['EUR'] ?? null, 'date'],
            ];
            ...
            foreach ($allCurrencies as $currency) {
                if ($currency['value'] !== null) {
                    Currency::updateOrCreate(
                        ['name' => $currency['name'], 'date' => now()->toDateString()],
